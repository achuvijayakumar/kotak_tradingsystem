...
class XTSCommon:
    ...
    def __init__(self, token=None, userID=None, isInvestorClient=None):
        ...
        #print(f"setting parent class {token} - {userID} - {isInvestorClient}")
        self.token = token
        self.userID = userID
...
class XTSConnect(XTSCommon):
    ...
    def __init__(self,
                 ...
                 secretKey,
                 source,
                 # root="https://xts.compositedge.com",
    ...
    def _set_common_variables(self, access_token, isInvestorClient):
        ...
        super().__init__(access_token, isInvestorClient=isInvestorClient)
    ...
    def interactive_login(self):
        ...
        print("[{}] user.login called".format(datetime.datetime.now()))
        print("with params user key {} secretkey {} source {}".format(self.apiKey, self.secretKey, self.source))
        response = {}
        try:
            params = {
                "appKey": self.apiKey,
            ...
            print("Exception while login xts {}".format(e))
    ...
    def get_order_book(self, clientID=None):
        ...
        try:
            print("[{}] order.status called".format(datetime.datetime.now()))
            params = {}
            if not self.isInvestorClient:
                if self.prop_account:
                    params['clientID'] = '*****'
                else:
    ...
    def place_order(self,
        ...
        """To place an order"""
        try:
            print("[{}] order.place called".format(datetime.datetime.now()))
            ...
            if not self.isInvestorClient:
                if self.prop_account:
                    ...
                    params['clientID'] = clientID
            print(f"isprop {self.prop_account} sending params for place order {params}")
    ...
    def get_balance(self, clientID=None):
        ...
        print(f"self.isInvestorClient is {self.isInvestorClient}")
        print("[{}] user.balance called".format(datetime.datetime.now()))
        #if self.isInvestorClient:
        try:
            params = {}
            if not self.isInvestorClient:
                print(f"encoding clientID is {clientID}")
                params['clientID'] = clientID
            else:
                print("Investor client is true. NOT encoding client id")
        ...
        #          "terminal")
        #    return {"type": "success",
        #            "result": {"BalanceList": [{"limitObject": {"RMSSubLimits": {"cashAvailable": 0}}}]}
        #            }
    ...
    def modify_order(self,
        ...
        try:
            print("[{}] order.modify called".format(datetime.datetime.now()))
    ...
    def get_position_netwise(self, clientID=None):
        ...
        try:
            print("[{}] post.position called".format(datetime.datetime.now()))
    ...
    def cancel_order(self, appOrderID, orderUniqueIdentifier, clientID=None):
        ...
        try:
            print("[{}] order.cancel called".format(datetime.datetime.now()))
    ...
    def cancelall_order(self, exchangeSegment, exchangeInstrumentID):
        ...
        try:
            print("[{}] order.cancelall called".format(datetime.datetime.now()))
    ...
    def get_order_history(self, appOrderID, clientID=None):
        ...
        to Filled etc """
        try:
            print("[{}] order.history called".format(datetime.datetime.now()))
            params = {'appOrderID': appOrderID}
            print(f"client id is {clientID}")
            print(f"is investor client is {self.isInvestorClient}")
            if not self.isInvestorClient:
                if self.prop_account:
                    params['clientID'] = '*****'
                else:
    ...
    def _request(self, route, method, parameters=None):
        ...
        print("[{}] {} {} called".format(datetime.datetime.now(), route, method))
